...
Chapter::Chapter(std::string& unparsed_string){
	vector<string> parsed_string = split_line(unparsed_string, ',');
...
string Chapter::to_string(){
	...
	for (int i = 0; i < versions.size(); ++i){
		result += ',' + int_to_string(versions[i]->num_of_commands()); // to know size of commands of each version
		result += ',' + versions[i]->to_string();
	}	
	return result;
}
